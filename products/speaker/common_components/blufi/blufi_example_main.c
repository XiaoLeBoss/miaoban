 ...
 * 
 * @param[in] rssi 信号强度
 * @param[in] reason 连接结束原因
 * 
 * @return void 无返回值
 * 
 * @note 根据当前是否处于连接状态，记录最大重试次数或连接结果信息
 */
 ...
 * @param void 无参数
 * 
 * @return bool 是否成功发起重连
 * 
 * @note 仅在重试次数未达上限时尝试重连
 */
...
/**
 * @brief 获取当前 SoftAP 连接的设备数量
 * 
 * @param void 无参数
 * 
 * @return int 当前连接的站点数量，失败返回 0
 */
 ...
 * @return void 无返回值
 * 
 * @note 处理获取 IP 事件，向 BluFi 发送连接报告
 */
 ...
 * @return void 无返回值
 * 
 * @note 处理 STA 连接/断开、扫描完成、AP 站点连接/断开等事件
 */
 ...
 * @return void 无返回值
 * 
 * @note 初始化网络接口、事件循环，注册 Wi-Fi/IP 事件处理函数，并启动 Wi-Fi
 */
 ...
 * @return void 无返回值
 * 
 * @note 处理各种 BluFi 事件，如初始化完成、连接请求、数据接收等
 */
static void example_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
 ...
 * @return void 无返回值
 * 
 * @note 初始化 NVS、Wi-Fi、BluFi 控制器和主机，并打印 BluFi 版本
 */
void app_main(void)
{
    esp_err_t ret;
