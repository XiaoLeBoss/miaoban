...
#include "system.hpp"
#include "audio.hpp"
#include "ai_framework/agent/coze_chat_app.hpp"
...
bool system_init()
{
    ...
    if (app_settings)
    {
        ...
        // Register audio resume callback
        // app_settings->manager.registerAudioResumeCallback([]()
        //                                                   {
        //     ESP_UTILS_LOGI("Resuming audio tasks...");
        //     if (audio_manager_suspend(false) != ESP_OK) {
        //         ESP_UTILS_LOGE("Audio resume failed");
        //     }
        //     if (!audio_init()) {
        //         ESP_UTILS_LOGE("Initialize audio failed");
        //     }
        //     if (coze_chat_app_init() != ESP_OK) {
        //         ESP_UTILS_LOGE("Init chat failed");
        //     } });
...
static bool check_whether_enter_developer_mode()
{
...

    {
        ...
        lv_obj_add_event_cb(exit_button, [](lv_event_t *e)
            ...
            esp_restart(); }, LV_EVENT_CLICKED, nullptr);
    ...
    case BUTTON_SINGLE_CLICK:
        if (_agent->hasChatState(Agent::ChatState::ChatStateStarted))
        {
            if (_agent->isChatState(Agent::ChatState::ChatStateSlept))
            {
                ...
                {
                    auto afe = audio_processor_get_afe_handle();
                    if (afe)
                    {
