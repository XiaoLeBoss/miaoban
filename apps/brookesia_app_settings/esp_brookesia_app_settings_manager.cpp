...
#define BLUFI_SW_FLAG_DEFAULT (0)
...
namespace esp_brookesia::apps
{
    ...
    extern "C" void blufi_app_start(void);
    extern "C" void blufi_app_stop(void);
    ...
    SettingsManager::SettingsManager(App &app_in, SettingsUI &ui_in, const SettingsManagerData &data_in) : app(app_in),
                                                                                                           ui(ui_in),
                                                                                                           data(data_in),
    ...
    bool SettingsManager::processInit()
    {
        ...
        ESP_UTILS_CHECK_FALSE_EXIT(manager->processAppEventOperation(*operation_data), "Process app event failed"); }, this);
    ...
    bool SettingsManager::processRunUI_ScreenSettings()
    {
        ...
        // More: Restart
        {
            ...
            esp_restart();
            return true; }, restart_cell->getClickEventID(), nullptr),
                false, "Register event failed");
        }
        ...
        // More: Developer mode
        {
            ...
            return true; }, developer_mode_cell->getClickEventID(), this),
                false, "Register event failed");
        }
    ...
    bool SettingsManager::processRunUI_ScreenWlan()
    {
        ...
        // Process BluFi switch
        lv_obj_t *blufi_sw = ui.screen_wlan.getElementObject(
            static_cast<int>(SettingsUI_ScreenWlanContainerIndex::PROVISIONING),
            static_cast<int>(SettingsUI_ScreenWlanCellIndex::PROVISIONING_BLUFI),
            SettingsUI_WidgetCellElement::RIGHT_SWITCH);
        ESP_UTILS_CHECK_NULL_RETURN(blufi_sw, false, "Get BluFi switch failed");
        StorageNVS::Value blufi_sw_flag;
        int blufi_sw_flag_int = BLUFI_SW_FLAG_DEFAULT;
        if (StorageNVS::requestInstance().getLocalParam(Manager::SETTINGS_BLUFI_SWITCH, blufi_sw_flag))
        {
            ESP_UTILS_CHECK_FALSE_RETURN(std::holds_alternative<int>(blufi_sw_flag), false, "Invalid BluFi switch flag type");
            blufi_sw_flag_int = std::get<int>(blufi_sw_flag);
        }
        else
        {
            ESP_UTILS_CHECK_FALSE_RETURN(
                StorageNVS::requestInstance().setLocalParam(Manager::SETTINGS_BLUFI_SWITCH, blufi_sw_flag_int, this), false,
                "Set BluFi switch flag failed");
        }
        if (blufi_sw_flag_int)
        {
            lv_obj_add_state(blufi_sw, LV_STATE_CHECKED);
        }
        else
        {
            lv_obj_clear_state(blufi_sw, LV_STATE_CHECKED);
        }
        lv_obj_add_event_cb(blufi_sw, onUI_ScreenWlanBluFiSwitchChangeEvent, LV_EVENT_VALUE_CHANGED, this);
    ...
    bool SettingsManager::toggleWlanScanTimer(bool is_start, bool is_once)
    {
        ...
        if (_disable_wlan_scan)
        {
            ESP_UTILS_LOGD("Scan disabled, skip toggle start");
            is_start = false;
        }
    ...
    bool SettingsManager::processOnWlanScanTimer(lv_timer_t *t)
    {
        ...
        if (_disable_wlan_scan)
        {
            ESP_UTILS_LOGD("Scan disabled, ignore scan start");
            return true;
        }
    ...
    bool SettingsManager::processOnWlanOperationThread()
    {
        ...
        _wlan_operation_start_cv.wait(start_lock, [this]
                                      { return !_wlan_operation_queue.empty(); });
    ...
    bool SettingsManager::processOnWlanUI_Thread()
    {
        ...
        auto event_id = is_wifi_event ? static_cast<int>(std::get<wifi_event_t>(wlan_event)) : static_cast<int>(std::get<ip_event_t>(wlan_event));
        ...
        if (is_wifi_event)
        {
            ...
            case WIFI_EVENT_STA_START:
                ...
                if (!_ui_wlan_softap_visible && !_disable_wlan_scan)
                {
     ...
     * @brief 处理WLAN总开关变更事件
     *
     * @param e LVGL事件对象，包含开关组件和事件信息
     * @return true 处理成功
     * @return false 处理失败（通常由于参数无效或内部操作失败）
     *
     * @note 此函数在UI线程中执行，负责更新WLAN开关状态、UI显示、触发底层WLAN操作以及联动BluFi配网开关。
     */
    bool SettingsManager::processOnUI_ScreenWlanControlSwitchChangeEvent(lv_event_t *e)
    {
        ...
        // 显示/隐藏状态栏WLAN图标
        // 根据开关状态设置状态栏图标为断开连接（显示图标）或关闭（隐藏图标）
        app.getSystem()->getDisplay().getQuickSettings().setWifiIconState(
        ...
        if (wlan_sw_flag)
        {
            // 如果WLAN开启，自动开启BluFi配网开关并启动BluFi服务
            lv_obj_add_state(blufi_sw, LV_STATE_CHECKED);
            ESP_UTILS_CHECK_FALSE_RETURN(
                StorageNVS::requestInstance().setLocalParam(Manager::SETTINGS_BLUFI_SWITCH, 1, this), false,
                "Set BluFi switch flag failed");
            blufi_app_start();
        }
        else
        {
            // 如果WLAN关闭，自动关闭BluFi配网开关并停止BluFi服务
            lv_obj_clear_state(blufi_sw, LV_STATE_CHECKED);
            ESP_UTILS_CHECK_FALSE_RETURN(
                StorageNVS::requestInstance().setLocalParam(Manager::SETTINGS_BLUFI_SWITCH, 0, this), false,
                "Set BluFi switch flag failed");

            // 停止WLAN扫描相关逻辑
            _disable_wlan_scan = true;
            toggleWlanScanTimer(false);

            // 强制停止WLAN扫描，timeout=0表示不等待
            forceWlanOperation(WlanOperation::SCAN_STOP, 0);

            // 创建线程异步停止BluFi，防止阻塞UI线程
            {
                esp_utils::thread_config_guard thread_config(esp_utils::ThreadConfig{
                    .name = "BLUFI_STOP",
                    .stack_size = 6 * 1024,
                ...
                boost::thread([this]()
                            ...
                            toggleWlanScanTimer(true, true); })
                                  ...
                                  ESP_UTILS_LOGI("After BluFi stop: SRAM free %uB (largest %uB), PSRAM free %uB (largest %uB)",
                                                 (unsigned)internal_free, (unsigned)internal_largest,
                                                 (unsigned)external_free, (unsigned)external_largest);

                                  //通过回调恢复音频
                                  if (audio_manager_is_suspended() && _audio_resume_callback)
                                  {
                                      _audio_resume_callback();
                                  } })
                    .detach();
            }
        }
    ...
    /**
     * @brief 处理BluFi配网开关变更事件
     *
     * @param e LVGL事件对象
     * @return true 处理成功
     * @return false 处理失败
     *
     * @note 负责单独控制BluFi功能的开启和关闭。
     */
    bool SettingsManager::processOnUI_ScreenWlanBluFiSwitchChangeEvent(lv_event_t *e)
    {
        ESP_UTILS_LOG_TRACE_GUARD_WITH_THIS();
        ...
        lv_state_t state = lv_obj_get_state(sw);
        bool blufi_sw_flag = state & LV_STATE_CHECKED;
        ...
        if (blufi_sw_flag)
        {
            ...
            _disable_wlan_scan = true;
            toggleWlanScanTimer(false);
            // 停止WLAN扫描，等待超时时间为WLAN_SCAN_STOP_WAIT_TIMEOUT_MS
            forceWlanOperation(WlanOperation::SCAN_STOP, WLAN_SCAN_STOP_WAIT_TIMEOUT_MS);
            blufi_app_start();
        }
        else
        {
            ...
            _disable_wlan_scan = true;
            toggleWlanScanTimer(false);
            // 停止WLAN扫描，timeout=0不等待
            forceWlanOperation(WlanOperation::SCAN_STOP, 0);

            // 创建线程异步停止BluFi，避免阻塞UI
            {
                esp_utils::thread_config_guard thread_config(esp_utils::ThreadConfig{
                    .name = "BLUFI_STOP",
                    .stack_size = 6 * 1024,
        ...
        // 保存BluFi开关状态到NVS
        ESP_UTILS_CHECK_FALSE_RETURN(
            StorageNVS::requestInstance().setLocalParam(Manager::SETTINGS_BLUFI_SWITCH, static_cast<int>(blufi_sw_flag), this), false,
            "Set BluFi switch flag failed");
    ...
    /**
     * @brief BluFi开关变更事件的静态回调入口
     *
     * @param e LVGL事件对象
     *
     * @note 此函数作为C风格的回调接口，将事件转发给C++成员函数 processOnUI_ScreenWlanBluFiSwitchChangeEvent
     */
    void SettingsManager::onUI_ScreenWlanBluFiSwitchChangeEvent(lv_event_t *e)
    {
        ESP_UTILS_LOG_TRACE_GUARD();

        ESP_UTILS_CHECK_NULL_EXIT(e, "Invalid event");
        ESP_UTILS_CHECK_NULL_EXIT(e->user_data, "Invalid user data");

        SettingsManager *manager = static_cast<SettingsManager *>(e->user_data);
        ESP_UTILS_CHECK_NULL_EXIT(manager, "Invalid manager");

        ESP_UTILS_CHECK_FALSE_EXIT(manager->processOnUI_ScreenWlanBluFiSwitchChangeEvent(e), "Process BluFi switch change failed");
    }

    /**
     * @brief 处理WLAN可用列表项点击事件
     *
     * @param data 事件处理数据，包含触发事件的对象
     * @return true 处理成功
     * @return false 处理失败
     *
     * @note 当用户点击WLAN扫描列表中的某个网络时触发，负责发起连接流程。
     */
    bool SettingsManager::processOnUI_ScreenWlanAvailableCellClickEvent(const systems::base::Event::HandlerData &data)
    {
        ...
        // 使用互斥锁保护WLAN数据访问，记录当前尝试连接的WLAN信息
        {
            std::lock_guard<std::mutex> lock(_ui_wlan_available_data_mutex);
    ...
    bool SettingsManager::processOnWlanEventHandler(WlanEvent event, void *event_data)
    {
        ...
        auto event_id = is_wifi_event ? static_cast<int>(std::get<wifi_event_t>(event)) : static_cast<int>(std::get<ip_event_t>(event));
        ...
        if (is_wifi_event)
        {
            ...
            case WIFI_EVENT_STA_DISCONNECTED:
            {
                ...
                bool need_check_retry = (!_is_wlan_force_connecting &&
